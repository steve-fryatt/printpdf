...
struct ps2paper_size {
	...
	osbool			ps2_file_exists;		/**< TRUE if the PS2 file exists; else FALSE	*/
...
static void ps2paper_pane_redraw_handler(wimp_draw *redraw)
{
	...
	while (more) {
		...
		for (y = top; y < paper_count && y <= base; y++) {
			...
			snprintf(buffer, sizeof(buffer), "%.3f", (double) (redraw_list[y])->width / 72000.0);
			...
			snprintf(buffer, sizeof(buffer), "%.3f", (double) (redraw_list[y])->height / 72000.0);
			...
			if ((redraw_list[y])->ps2_file_exists)
	...
	while (fgets(line, sizeof(line), in) != NULL) {
		...
		if (paper_name != '\0' && paper_width != 0 && paper_height != 0) {
			...
			if (paper_definition != NULL) {
				...
				paper_definition->ps2_file_exists = FALSE;
				...
				if (paper_definition->ps2_file[0] != '\0') {
					...
					if (error == NULL && type == fileswitch_IS_FILE)
						paper_definition->ps2_file_exists = TRUE;
				}
